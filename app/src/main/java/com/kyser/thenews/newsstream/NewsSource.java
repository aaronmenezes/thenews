...
public class NewsSource {
    ...
    public MutableLiveData<NewsResponse> getSearchResult(String search_key){
        MutableLiveData<NewsResponse>  newsResponseMutableLiveData = new MutableLiveData<>();
        mService.getEverything("6069225c835e4bfe800d3de6eb0b36fd",search_key,"100").enqueue(new Callback<NewsResponse>() {
            @Override
            public void onResponse(Call<NewsResponse> call, Response<NewsResponse> response) {
                Log.v("=======","-----"+response);
                newsResponseMutableLiveData.setValue(response.body());
